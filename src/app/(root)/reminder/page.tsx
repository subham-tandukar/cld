...
export async function generateMetadata(parent: ResolvingMetadata): Promise<Metadata> {
    ...
    const title = `${data?.data?.reminder_title_np} - Online Khabar Calendar`
    const description = removeHTMLTags(data?.data?.reminder_description_np)
    const ogTitle = `${title}`
    ...
    return {
        ...
        openGraph: {
           ...
           images: [imageUrl, ...previousImg],
            url: `https://calendar.onlinekhabar.com/reminder`
        },
        twitter: {
            title: ogTitle,
            description: description,
