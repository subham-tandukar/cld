...
export async function generateMetadata(parent: ResolvingMetadata): Promise<Metadata> {
    ...
    const title = `${data?.data?.upcoming_events_title_np} - Online Khabar Calendar`
    const description = removeHTMLTags(data?.data?.upcoming_events_description_np)
    const ogTitle = `${title}`
    const imageUrl = `${data?.data?.upcoming_events_thumbnail ? data?.data?.upcoming_events_thumbnail : data?.data?.main_logo}`
    ...
    return {
        ...
        openGraph: {
           ...
           images: [imageUrl, ...previousImg],
            url: `https://calendar.onlinekhabar.com/upcoming-event`
        },
        twitter: {
            title: ogTitle,
            description: description,
