...
export async function generateMetadata(parent: ResolvingMetadata): Promise<Metadata> {
    ...
    const title = `${data?.data?.sahit_title_np} - Online Khabar Calendar`
    const description = removeHTMLTags(data?.data?.sahit_description_np)
    const ogTitle = `${title}`
    const imageUrl = `${data?.data?.sahit_thumbnail ? data?.data?.sahit_thumbnail : data?.data?.main_logo}`
    ...
    return {
        ...
        openGraph: {
            ...
            url: `https://calendar.onlinekhabar.com/sahit`
        },
        twitter: {
            title: ogTitle,
            description: description,
