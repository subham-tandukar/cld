...
export async function generateMetadata(parent: ResolvingMetadata): Promise<Metadata> {
    ...
    const title = `${data?.data?.events_title_np} - Online Khabar Calendar`
    const description = removeHTMLTags(data?.data?.events_description_np)
    const ogTitle = `${title}`
    const imageUrl = `${data?.data?.events_thumbnail ? data?.data?.events_thumbnail : data?.data?.main_logo}`
    return {
        ...
        openGraph: {
            ...
            url: `https://calendar.onlinekhabar.com/event`
        },
        twitter: {
            title: ogTitle,
            description: description,
